...
const DashboardPage = () => {
    ...
    // WebSocket Logic (Strict Matching)
    useEffect(() => {
        ...
        wsRef.current.onmessage = (event) => {
            try {
                ...
                if (message.type === 'GPS_UPDATE') {
                    ...
                    // Extracts "L7" from "L7: Terminus..."
                    const wsRef = wsLineCode.split(':')[0].trim(); 
                    
                    // Matches if WS is "L7" and Route is "7", or Exact Match "32H" === "32H"
                    const isMatch = (wsRef === routeLineRef) || (wsRef === `L${routeLineRef}`);
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden bg-slate-100">
            ...
            <MapContainer center={[33.9716, -6.8498]} zoom={13} className="w-full h-full z-0" zoomControl={false}>
